...
```javascript
<EmojiPicker defaultGroup="foodDrink" defaultSkinTone="medium" />
```
...
`icons` ([list][icons]) prop. By default the picker uses native emoji but can render React
components.
...
styles. This is necessary for accurately computing widths and heights.
