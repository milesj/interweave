...
export class InternalPicker extends React.PureComponent<InternalPickerProps, InternalPickerState> {
	...
	constructor(props: InternalPickerProps) {
		...
		const { blockList, classNames, defaultSkinTone, messages, allowList } =
	...
	generateAllowBlockMap(list: string[]): AllowBlockMap {
		...
		list.forEach((hexcode) => {
			if (__DEV__ && hexcode.match(SKIN_MODIFIER_PATTERN)) {
	...
	setUpdatedState(nextState: Partial<InternalPickerState>, forceRebuild: boolean = false) {
		...
		this.setState((prevState) => {
			...
			if (
				'activeGroup' in nextState && // Reset search query
				state.searchQuery
			) {
				state.searchQuery = '';
				rebuildEmojis = true;
	...
	override render() {
		...
		const components: Record<string, React.ReactElement<unknown> | null> = {
			emojis: (
				<EmojiList
					key="emojis"
					{...virtual}
...
export function Picker({
	...
	return <InternalPicker {...props} emojiData={data} emojis={emojis} emojiSource={source} />;
}
